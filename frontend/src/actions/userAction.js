...
import { CLEAR_CART } from "../constants/cardConstant";
...
export const updatePassword =(password)=>async(dispatch)=>{
  try {
    ...
    const {data} = await axios.patch("/api/v1/users/password/update",password,config);
    dispatch({
...
export const resetPassword =(token,password)=>async(dispatch)=>{
  try {
    ...
    const {data} = await axios.post(`/api/v1/users/resetPassword/${token}`,password,config);
